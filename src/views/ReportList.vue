<script setup>
    ...
    const { onFetchResponse, data, execute } = useFetch('/api/student/report-post?' + qs.stringify({ user_id: userData.user_id }), {
        immediate: true,
    }).get().json();
                            ...
                            <el-table :data="tableData"
                                ...
                                <el-table-column label="Status">
                                    <template #default="scope">
                                        <el-icon v-if="scope.row.status==0"><SemiSelect /></el-icon>
                                        <el-icon v-if="scope.row.status==1"><Select /></el-icon>
                                        <el-icon v-if="scope.row.status==2"><CloseBold /></el-icon>
                                    </template>
                                </el-table-column>
